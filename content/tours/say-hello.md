...
- verify: Arrive here from the previous step: the compose panel should already be open, the message box should hold a short draft with a line on the card saying the tour staged it, and Send should be untouched. Open /mail#compose in a fresh tab, with no tour running: the panel should be open on arrival, no click. Then type over the draft: your words replace the tour's and stay for the rest of the walk, because a field you have touched is yours. Back does not test that, and never could: the step before this one lives on the home page, so walking back leaves /mail entirely and the next arrival is a fresh page.

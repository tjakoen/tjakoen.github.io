...
// inlineText(parsed heading children) (MILL's default grain-adapter, since 0.2.0). They are
// different implementations, so a heading they disagree on would emit a chunk anchor that matches
// NO rendered id — and the deep-link would silently 404 its scroll. This round-trip test walks the
// REAL corpus sources and asserts every anchor buildKnowledge emits exists among the ids the
// renderer would stamp on that source's page. New content that breaks the contract fails here, at
// build time, not in a visitor's browser.
...
describe("chunk anchors match rendered heading ids (real corpus)", () => {
  test("every emitted anchor has a rendered id on its page", async () => {
    ...
    for (const src of sources) {
      // the ids mill's grain-adapter would stamp (h2/h3 only, same as the corpus split)

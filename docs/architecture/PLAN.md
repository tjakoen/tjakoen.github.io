...
- **Docs = the `docs/*.md` we already maintain, rendered — not new prose (portfolio-wide).** Each
  ...
  **markdown content collection (piece 3)** — no new pipeline, no new prose. The showcase's concept
  ...
  `view/pages/grain/GRAIN-PAGE.md` and `view/pages/batch/BATCH-PAGE.md`.
...
- **Separation of concerns:** shell **primitives** (rail incl. grouped variant, `tab-bar`,
  ...
  **custom theme/components** live in **`tjakoen.github.io/`** — new `tjakoen.github.io/view/components/` (register in
...
3. **Welcome page**: promote `/welcome-poc` to `/` with components in `tjakoen.github.io/view/components/**`,
...
2. **The scene** — rebuild `tjakoen.github.io/pages/index.html` as the desk; object styling as
   portfolio components (`tjakoen.github.io/view/components/**` — site-specific, NOT grain). Server-render the
...
- **The GRAIN showcase now lives here, at `/grain`** (`tjakoen.github.io/pages/grain/index.html`) — moved
  ...
  self-documentation; the narrative showcase is a portfolio section. See `view/pages/grain/GRAIN-PAGE.md`.
- **The BATCH showcase is the companion section, at `/batch`** — the pitch for the substrate
  ...
  the export pipeline. See `view/pages/batch/BATCH-PAGE.md`.

...
→ *how:* [CONVENTIONS §3](batch/CONVENTIONS.md) · [ARCHITECTURE §17](batch/ARCHITECTURE.md) · [AI-INTERFACE.md](grain/AI-INTERFACE.md)
...
[the whitepaper](../content/notes/whitepaper-one-vocabulary.md).
...
- **Built for a human, more so for an AI.** Every surface must be easy for a person *and even more*
  ...
  it's the half we optimize for. → [AI-INTERFACE.md](grain/AI-INTERFACE.md)
- **A mistake is a design signal, not just a bug.** When something breaks, a check flags, or an AI (or
  ...
  that live this out: [grain/CLAUDE.md](../../grain/packages/grain/CLAUDE.md) (lesson 5), [batch/CLAUDE.md](../../batch/CLAUDE.md).
- **No build step.** The server *is* the build step — it composes final HTML on every request.
  ...
  second renderer. → [ARCHITECTURE §0.5](batch/ARCHITECTURE.md), [§18](batch/ARCHITECTURE.md)
- **Native-first.** Standards → runtime-native → library, in that order. Modern HTML/CSS/JS are
  ...
  primitive* over reinventing it. → [ARCHITECTURE §0](batch/ARCHITECTURE.md),
  [§11.3](batch/ARCHITECTURE.md), [CONVENTIONS §2](batch/CONVENTIONS.md),
  [GRAIN.md](grain/GRAIN.md) ("What GRAIN gives you")
- **Hypermedia, server-rendered.** Fragments over the wire, htmx for reads/nav; avoids a client
  framework, a build, and client/server state-sync bugs. → [ARCHITECTURE §0.5](batch/ARCHITECTURE.md)
...
- **The AI gets a modality, not a chat channel.** A finite vocabulary of real action primitives —
  ...
  now*), and the surface's own affordances are its **physics**. → [AI-INTERFACE.md](grain/AI-INTERFACE.md)
- **Grade-as-signal.** Type grade carries meaning: **grain = AI / in-transit**, clean = human /
  committed. The AI's presence is always shown, never hidden. → [CONVENTIONS §4.5](batch/CONVENTIONS.md) ·
  [DESIGN-SYSTEM.md](grain/DESIGN-SYSTEM.md)
- **Tokens only.** No hardcoded colors, ever. Components read semantic `var(--token)`s; re-skinning
  repoints tokens in one place, never edits a component. → [CONVENTIONS §5](batch/CONVENTIONS.md)
- **Atomic design, credited.** Components compose bottom-up as atoms → molecules → organisms — the
  ...
  the taxonomy deliberately; the atomic layer lives in GRAIN. → [ARCHITECTURE §2](batch/ARCHITECTURE.md) ·
  [DESIGN-SYSTEM.md](grain/DESIGN-SYSTEM.md)
- **Layers point inward; seams are ports.** `batch` imports nothing outward; `grain` imports nothing
  ...
  port, don't reach. → [CONVENTIONS §1](batch/CONVENTIONS.md), [§10](batch/CONVENTIONS.md)
- **Tests are part of the work.** Three tiers — unit / integration / e2e — written as you build, not
  after. → [CONVENTIONS §6](batch/CONVENTIONS.md)
...
→ canonical diagram + rules: [CLAUDE.md](../../bread/CLAUDE.md) · [CONVENTIONS §1](batch/CONVENTIONS.md)
...
| substrate mechanism + trade-offs | [ARCHITECTURE.md](batch/ARCHITECTURE.md) |
| build rules (layering, tokens, testing, vocabulary) | [CONVENTIONS.md](batch/CONVENTIONS.md) |
| AI contract (surfaces, ops, manifest, "AI acts") | [AI-INTERFACE.md](grain/AI-INTERFACE.md) |
